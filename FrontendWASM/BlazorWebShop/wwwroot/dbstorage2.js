export function synchronizeFileWithIndexedDb(filename) {
    return new Promise((res, rej) => {
        ...
        db.onsuccess = () => {
            const req = db.result.transaction('Files', 'readonly').objectStore('Files').get('products')
            req.onsuccess = () => {
        ...
        setInterval(() => {
            ...
            if (FS.analyzePath(path).exists) {
                ...
                if (mtime.valueOf() !== lastModifiedTime.valueOf()) {
                    ...
                    db.result.transaction('Files', 'readwrite').objectStore('Files').put(data, 'products');
                }
            }
        }, 1000);
    });
